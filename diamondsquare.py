...
def diamond_square(desired_size, min_height, max_height, roughness, chaos=0, random_seed=None, AS_NP_ARRAY=False, USE_NEW_SQUARE_STEP=True):
    ...
    a = np.min(DS_array[:])
    b = np.max(DS_array[:])
    DS_array = min_height + ((DS_array-a) / (b-a) * (max_height - min_height))
